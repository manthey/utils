...
-v increases verbosity.""")
    ...
    for line in info:
        if re.match(r'^\s+Stream #.+?: Video', line) and not origw:
        ...
        if re.match(r'^\s+Stream #.+?: Audio', line) and not origakbps:
            for part in line.split(',')[1:]:
        ...
        if re.match(r'^\s+Duration: ', line) and not origdur:
    ...
    if opts.get('crop', False) is True and origdur and 'seq' not in opts:
        cmd = [os.path.join(ProgramRoot, 'ffmpeg')]
        cmd.extend(['-i', opts['files'][0]])
    ...
    if 'seq' not in opts:
        ...
        if 'duration' in opts:
            ...
            cmd.extend(['-t', '%5.3f' % ((duration_parse(opts['end']) -
                       duration_parse(opts.get('start', 0))) / factor)])
    w = None
    kbps = None
    audio_kbps = None
    ...
    if format == 'mp4':
        ...
        cmd.extend([
            '-flags', '+loop', '-subq', '4', '-trellis', '0', '-refs', '1',
            '-coder', '0', '-me_method', 'hex', '-me_range', '16',
            '-keyint_min', '15', '-sc_threshold', '40', '-i_qfactor', '0.71',
            '-bt', '200k', '-rc_eq', 'blurCplx^^^(1-qComp^)', '-qcomp', '0.6',
            '-qmin', '10', '-qmax', '51', '-qdiff', '2', '-g', '15', '-async',
            '2', '-bf', '0', '-deblock', '2:2'])
    ...
    if format == 'mp4':
        ...
        if orighz and orighz not in [44100, 48000]:
            cmd.extend(['-ar', '48000'])
    else:
